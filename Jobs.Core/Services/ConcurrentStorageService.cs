...
public class ConcurrentStorageService<T> : IConcurrentStorageService<T> where T : class
{
    ...
    private static readonly ConcurrentStorageService<T> StorageService = new();
    ...
    public bool AddOrUpdate(string key, T value)
    {
        if (_concurrentDictionary.ContainsKey(key))
        {
            ...
            //_concurrentDictionary.AddOrUpdate(key, _concurrentDictionary[key], (k,ov) => value);
        }
        
        //_concurrentDictionary.AddOrUpdate(key, value, (k,ov) => ov);
